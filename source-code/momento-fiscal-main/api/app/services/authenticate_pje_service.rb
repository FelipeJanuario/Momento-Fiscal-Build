...
class AuthenticatePjeService
  ...
  def initialize(
    ...
    @pje_pfx = pje_pfx
    @pje_cert_chain = parse_cert_chain(pje_cert_chain)
    @password = password
    @cookie_header = nil
    @tab_id = nil
  ...
  def validate_args
    raise ArgumentError, "pje_pfx or PJE_AUTH_PFX environment variable is required" if @pje_pfx.nil? || @pje_pfx.empty?
    raise ArgumentError, "password or PJE_AUTH_PFX_PASSWORD environment variable is required" if @password.nil? || @password.empty?
    raise ArgumentError, "pje_cert_chain or PJE_AUTH_CERTCHAIN environment variable is required" if @pje_cert_chain.nil? || @pje_cert_chain.empty?
  end
  ...
  def find_tab_id(response)
    ...
    @tab_id ||= response["location"].match(/tab_id=([^&]+)/)[1]
  end
  ...
  def fetch_auth_params
    ...
    Rails.logger.info { "[AuthenticatePjeService][fetch_auth_params] codigoSeguranca: #{codigo_seguranca}" }

    # Parse token and mensagem from the codigoSeguranca URL (pjeoffice:// protocol URL
    # generated by the SSO server per session). Using server-generated values is required
    # for the SSO to validate the signed challenge.
    pjeoffice_uri = URI.parse(codigo_seguranca) rescue nil
    pjeoffice_params = pjeoffice_uri ? URI.decode_www_form(pjeoffice_uri.query.to_s).to_h : {}

    message = pjeoffice_params["mensagem"] || "0.#{generate_random_chars(10)}"
    token   = pjeoffice_params["token"]    || SecureRandom.uuid

    Rails.logger.info { "[AuthenticatePjeService][fetch_auth_params] mensagem: #{message}, token: #{token}" }
  ...
  def sign_pje_message(params)
    ...
    # Sign the message using MD5 with RSA (required by PJeOffice REST)
  ...
  def send_signature(params, signed_data)
    ...
    Rails.logger.info { "[AuthenticatePjeService][send_signature] POST #{uri}" }
    Rails.logger.info { "[AuthenticatePjeService][send_signature] certChain type: #{@pje_cert_chain.class}, length: #{@pje_cert_chain.length}" }
    ...
    Rails.logger.info { "[AuthenticatePjeService][send_signature] Response: #{res.code} #{res.body&.slice(0, 200)}" }
